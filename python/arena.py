...
def compute_hidden_layer_weighted_input(x, hidden_layer_weights, hidden_layer_biases):
    ...
    return hidden_layer_weights @ x + hidden_layer_biases
...
def compute_output_layer_weighted_input(
    ...
    return output_layer_weights @ hidden_layer_activation + output_layer_biases
...
def compute_gradients(
    ...
    # fill in
    yhat = sigmoid(output_layer_weighted_input)[0][0]
    # refer to function (7)
    d_yhat = yhat - y  
    # refer to function (8) , shape (1, 1)
    dz2 = d_yhat * sigmoid_prime(output_layer_weighted_input)
    # refer to function (9), shape (1, 4)
    dw2 = dz2 * sigmoid(hidden_layer_weighted_input).T
    # refer to function (10) , shape (1, 1)
    db2 = dz2 * np.ones((1, 1))  

    # refer to function (11) , shape (4, 1)
    da1 = dz2 * output_layer_weights.T  
    # refer to function (12) , shape (4, 1)
    dz1 = da1 * sigmoid_prime(hidden_layer_weighted_input)
    # refer to function (13) , shape (4, 2)
    dw1 = dz1 @ x.T  
    # refer to function (14) , shape (4, 1)
    db1 = dz1 * np.ones((HIDDEN_LAYER_WIDTH, 1))
...
def get_new_weights_and_biases(
    ...
    for x, y in training_batch:
        hidden_layer_weighted_input = compute_hidden_layer_weighted_input(
            x, hidden_layer_weights, hidden_layer_biases)

        hidden_layer_activation = sigmoid(hidden_layer_weighted_input)

        output_layer_weighted_input = compute_output_layer_weighted_input(
            hidden_layer_activation, output_layer_weights, output_layer_biases
        )
        ...
        dw1.append(weight_gradients[0])
        dw2.append(weight_gradients[1])
        db1.append(bias_gradients[0])
        db2.append(bias_gradients[1])

    new_weights = [
        hidden_layer_weights - LEARNING_RATE * np.mean(dw1, axis=0),
        output_layer_weights - LEARNING_RATE * np.mean(dw2, axis=0)
    ]
    new_biases = [
        hidden_layer_biases - LEARNING_RATE * np.mean(db1, axis=0),
        output_layer_biases - LEARNING_RATE * np.mean(db2, axis=0),
        ]
...
def predict(
    ...
    hidden_layer_activation = sigmoid(compute_hidden_layer_weighted_input(
        x, hidden_layer_weights, hidden_layer_biases))
    output_layer_activation = sigmoid(compute_output_layer_weighted_input(
        hidden_layer_activation, output_layer_weights, output_layer_biases))
